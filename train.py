...
def discretized_gaussian_log_likelihood(x, *, means, log_scales): # ! not used?
...
def train(gpu, opt, output_dir, noises_init):
    ...
    if opt.model != '': # default as empty, set pretrained weights if continue to train
        ...
        for k, v in ckpt['model_state'].items():
            if k in model_dict and (v.shape == model_dict[k].shape):
                ...
                #     continue
                pretrained_dict[k] = v
        print("loaded layers: ", len(pretrained_dict))
        ...
        print("confirm loaded layers: ", count)
    ...
    for epoch in range(start_epoch, opt.niter):
        ...
        if (epoch + 1) % opt.diagIter == 0 and should_diag:
            ...
            if not opt.debug:
                if opt.use_tb:
                    tb_writer.add_scalar('total_bpd_b', kl_stats['total_bpd_b'].item(), epoch)
        ...
        if (epoch + 1) % opt.saveIter == 0:
            ...
            if should_diag:
                ...
                torch.save(save_dict, '%s/epoch_%d.pth' % (output_dir, epoch)) # save weights
...
def parse_args():
    ...
    parser.add_argument('--diagIter', default=50000, type=int, help='unit: epoch') # 50000 ?
    parser.add_argument('--vizIter', default=50000, type=int, help='unit: epoch') # 50000 ?
